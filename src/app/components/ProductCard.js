...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="max-w-sm bg-white rounded-2xl shadow-md hover:shadow-xl hover:scale-[1.02] transition-all duration-300 overflow-hidden">
      ...
      {/* Content Section – ছবির টেক্সট লেআউট অনুযায়ী */}
      <div className="p-5">
        {/* Product Name (বড় ও গাঢ়) */}
        <h3 className="text-xl font-bold text-gray-800">{product.name}</h3>
        
        {/* Brand + Stock (ছবির "EU38 BLACK/WHITE" এর জায়গায়) */}
        <p className="text-sm text-gray-500 mt-1">
          {product.brand} · Stock: {product.stock}
        </p>

        {/* Short Description (ছবির "Crossing hardwood comfort..." লাইনের মতো) */}
        <p className="text-gray-600 text-sm mt-3 line-clamp-2">
          {product.description}
        </p>

        {/* Price & View Details Button (দাম বামে, বাটন ডানে) */}
